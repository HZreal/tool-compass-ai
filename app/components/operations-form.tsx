...
export function OperationsForm() {
  ...
  return <div className="admin-lede">
    ...
    <section><h2>恢复备份</h2><p>恢复将覆盖当前业务数据。请先下载当前备份，再选择同版本备份并输入 RESTORE AI SCENERY。恢复支持 5 MB / 4999 行以内的 MVP 备份，另预留一行恢复审计。</p><form onSubmit={restore}><label>备份文件<input required type="file" name="backup" accept=".json,application/json" /></label><label>恢复确认<input required name="confirmation" autoComplete="off" placeholder="RESTORE AI SCENERY" pattern="RESTORE AI SCENERY" /></label><button className="button button--red" disabled={busy} type="submit">确认覆盖并恢复</button></form></section>
